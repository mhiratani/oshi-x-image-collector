...
Bearer Token・R2認証情報・Firebase設定は`EncryptedSharedPreferences`（暗号鍵が端末のKeystoreに紐付く）に保存されるため、Androidの自動バックアップやクラウド復元では新端末へ引き継げない。代わりに設定画面末尾の「設定の引き継ぎ（機種変更）」区画からエクスポート/インポートする。

- **エクスポート**: パスフレーズを決めると、全設定をAES-256-GCM（鍵はPBKDF2-HMAC-SHA256で導出）で暗号化したJSONファイルとして任意の場所（Google Drive・ダウンロード等）へ書き出す。
...
引き継ぎの全体手順: ①旧端末で設定をエクスポート → ②新端末にAPKをインストールし設定をインポート → ③Googleで再ログインし「クラウドから復元」でデータ本体を復元。
