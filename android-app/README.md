...
機種変更・端末紛失・アプリ再インストール時に、クラウド（Firestore + R2）に保存済みのデータを端末へ戻すための機能。上記の初回起動オンボーディングから選ぶか、既存ユーザーは設定 > バックアップ設定タブの「クラウドからの復元・同期」区画にある「クラウドから復元」ボタンから手動で実行できる（トグルがONの間だけ表示される）。実行結果は閉じるまで画面に残る。
...
引き継ぎの全体手順: ①旧端末の共通設定タブで設定をエクスポート → ②新端末にAPKをインストールし共通設定タブでインポート → ③バックアップ設定タブのトグルをONにしてGoogleで再ログイン → ④「クラウドから復元」でデータ本体を復元。
