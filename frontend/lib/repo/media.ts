...
export async function insertMediaBatch(
  ...
  for (const group of chunk(media, FIRESTORE_BATCH_LIMIT)) {
    const refs = group.map((m) => col(uid).doc(m.media_key));
...
export async function getMedia(uid: string, mediaKey: string): Promise<MediaRow | null> {
  const snap = await col(uid).doc(mediaKey).get();
...
export async function countForXUserId(uid: string, xUserId: string): Promise<number> {
  const agg = await col(uid).where('x_user_id', '==', xUserId).count().get();
...
export async function revealAll(uid: string, xUserIds: string[]): Promise<void> {
  for (const idsGroup of chunk(xUserIds, IN_QUERY_LIMIT)) {
    await updateQueryInBatches(
      col(uid).where('x_user_id', 'in', idsGroup).where('revealed', '==', false),
      { revealed: true }
    );
  }
}
...
export async function getOldestTweetId(uid: string, xUserId: string): Promise<string | null> {
  const snap = await col(uid).where('x_user_id', '==', xUserId).orderBy('posted_at', 'asc').limit(1).get();
...
export async function listPendingBackup(uid: string, batchSize: number): Promise<MediaRow[]> {
...
export async function markBackedUp(uid: string, mediaKey: string, r2BackupUrl: string): Promise<void> {
  await col(uid).doc(mediaKey).update({ r2_backup_url: r2BackupUrl, backed_up: true });
}

export async function markBackupFailed(uid: string, mediaKey: string): Promise<void> {
  await col(uid).doc(mediaKey).update({ backup_attempts: FieldValue.increment(1) });
}

// バックアップ済み・未判定(is_face IS NULL)・手動レビュー未実施の画像を判定対象として返す
export async function listPendingFaceDetection(uid: string, batchSize: number): Promise<MediaRow[]> {
  const snap = await col(uid)
